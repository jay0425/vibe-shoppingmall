# fe 환경 설정 및 UI 구현

1. 첨부한 컨벤션 참고해서 /client 폴더에 next.js, tailwindCSS, ts, tanstack-query,pnpm 조합으로 프로젝트 환경 셋팅 해줘.
2. /client 폴더 안에 app, components, lib, public 폴더들 다른 곳에서 복붙해왔어. 첨부한 컨벤션 내용 참고해서 여기 프로젝트에 맞게 수정해줘.

# 기능 연결

## 회원가입, 로그인

1. 루트 리드미 컨벤션 참고해서 작성해줘.

/client 폴더에서 FSD 폴더구조가 되게 해줘.

첨부한 컨트롤러를 참고해서
client/src/app/signup/page.tsx
에서 버튼 클릭 시 회원가입 되도록 로직 연결해줘.

/server 아래 파일들은 참고만 하고 건들지 말아줘.

그리고 만약에 API와 UI가 안맞는 부분이 있으면 나중에 보고 주고 일단 API 연결부터 해줘

2.  이메일 형식 검사,
    중복 가입 확인,
    비밀번호는
    8자 이상 20자 이하
    영문 + 숫자 + 특수문자 포함
    공백 불가
    허용된 특수문자 외 입력 차단
    password, 반복 문자, 단순 숫자 나열 등 차단

이름·이메일·비밀번호는 필수값,

모든 입력값은 공백 제거, 특수문자/이모지 불가

---

위의 내용을 반영해서 정규식 및 유효성 검사 로직 추가해줘.

3. 뒤로 가기 후 다시 가입 시도 시
   →회원가입폼이 초기화되어 입력하던 유저정보는 전부 리셋되어 있어야 함.
   가입 중에 인터넷 네트워크가 끊겨버림
   → 에러 상황에 따른 알맞은 유저가 이해할 수 있는 메세지 출력

---

위 내용도 반영해서 수정해줘.

4. client/src/app/login/page.tsx

위의 경로의 페이지에 첨부한 서버코드를 참고해서 로그인 기능을 연동해줘. 기존 FSD 폴더 구조 지켜서 만들어줘. 그리고 로그아웃 기능도 만들어줘.

5. client/src/app/page.tsx : 메인 페이지에서 토큰이 있으면 토큰을 가지고 유저 정보를 가져와서 우측 상단에 name님 반갑습니다. 메시지를 보여줘

## 상품 CRUD

1. admin/products/new 여기 페이지에 상품 등록 API를 연결해줘.
2. client/src/app/admin/products/page.tsx : 상품 리스트 조회 API 연결해줘
3. 이미지 추가를 cloudinary 위젯을 이용해서 업데이트 하고 싶어, 그리고 사진 업데이트 하면 미리보기로 볼 수 있게 하고 싶어.
4. .env에 클라우드 네임, 프리셋 값 넣음...
5. client/src/app/admin/products/page.tsx : 페이지네이션 버튼을 추가해서 페이지네이션 기능을 붙여 만들고 싶어. 한 페이지당 10 개씩 데이터 조회 시키고 싶어.
   페이지네이션 ui 컴포넌트 따로 파일분리해서 만들어 주고 그걸 가져다 이 페이지에서 조회 API 연결해서 10개 씩 보이게끔 만들어줘
6. client/src/app/products/[id]/page.tsx : 상품 조회 API 연결해줘

## 장바구니

1. cart 페이지에 cart API 연결해줘

## 주문

1. 주문 API 연결해줘
2. (포트원 주소 복붙함)
3. IMP.init을 이용해 포트원 결제 모듈 초기화 코드를 추가해줘 참고로 내 고객사 식별코드는 <~~> 야
4. client/src/app/checkout/page.tsx: PortOne V2 (inicis_v2) 방식으로 결제 연동해줘. PortOne.requestPayment() 를 이용해서 결제 코드 수정해줘.

## 관리자 주문 내역 확인

1. client/src/app/admin/orders/page.tsx

고객 주문 API 연동해줘
